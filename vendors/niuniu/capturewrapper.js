...
function Init(cb) {
    ...
    //设置控件加载完成以及截图完成的回调函数
    captureObj.FinishedCallback = cb;
...
function StartCapture() {
    ...
    if (captureRet == emCaptureFailed) {
        ...
        var elementDiv = '<div class="modal fade bs-example-modal-sm" id="new" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">' +
            ...
            ' <button type="button" class="btn btn-default" style="margin-bottom: 0" data-dismiss="modal">关闭</button><a target="_blank" class="btn btn-primary" href="../vendors/niuniu/CaptureInstall.exe">安装</a>' +
            '</div>' +
            '</div></div></div>';
        var modal = document.createElement("div");
        // '<a  href="../vendors/niuniu/CaptureInstall.exe" type="button" class="btn btn-primary">下载</a>' +
        modal.innerHTML = elementDiv;
        document.body.appendChild(modal);
        $("#new").modal("show");//data-dismiss="modal"
